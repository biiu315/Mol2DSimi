...
class similarity_validation:
    ...
    def plot_ap(self,precision, recall, ap, model, base_name):
        ...
        plt.xlim([0.0, 1.0])
        plt.ylim([0.0, 1.05])
        plt.xlabel('Rrecision', fontsize = 16)
    ...
    def validation(self):
        ...
        for i in self.simi_col:
            ...
            f1 = f1_score(self.data[self.active_col], self.data[i].round())
            self.list_f1.append(f1)
            gh = GH_score(self.data[self.active_col], self.data[i].round())
            self.list_GH.append(gh)
            
            
        index = ['Model', "AP", "AUCROC", "logAUCROC", "EF1%","EF5%", "EF10%", 'F1', 'GH']
        metric =[self.model,self.list_ap, self.list_roc_auc, self.list_log_roc_auc, self.list_ef1, self.list_ef5, self.list_ef10,
